 ...
 */
public class VarOperator implements OnlineAggregationOperation {

    static Logger logger = Logger.getLogger(VarOperator.class.getName());
    ...
    public boolean parseSQL(String query) {
        ...
        for (int i = 0; i < cols.length; i++) {
            if (cols[i].equalsIgnoreCase(Constants.SELECT)) {
                i++;
                if (i < cols.length && Constants.VAR.equalsIgnoreCase(cols[i].substring(0, 3))) {
                    ...
                    if (StrUtils.isDouble(strColumnIndex)) {
                        ...
                        logger.severe("parse variance column error:" + strColumnIndex);
                        return false;
                    }
                } else {
                    logger.info("std dev operation parse sql error:" + query + " not variance");
                    return false;
                }
    ...
    public Object exec(String query) {
        ...
        double sum = 0, avg = 0, power2Sum = 0, power2Avg = 0, power3Sum = 0, power4Sum = 0;
        ...
        while (true) {
            ...
            epsilon = Math.sqrt(zp * zp * gn1 / (count * tnu * tnu));
    ...
    public void showResult() {
        ...
        System.out.println("Interval：[" + variance + "-" + epsilon + "," + variance + "+" + epsilon + "]");
